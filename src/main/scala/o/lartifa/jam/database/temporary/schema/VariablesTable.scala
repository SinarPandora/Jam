...
trait VariablesTable {
  ...
  case class VariablesRow(id: Long, name: String, chatType: String, chatId: Long, value: String = "", `type`: String = "TEXT")
  ...
  implicit def GetResultVariablesRow(implicit e0: GR[Long], e1: GR[String]): GR[VariablesRow] = GR{
    prs => import prs._
    VariablesRow.tupled((<<[Long], <<[String], <<[String], <<[Long], <<[String], <<[String]))
  }
  ...
  class Variables(_tableTag: Tag) extends profile.api.Table[VariablesRow](_tableTag, "VARIABLES") {
    def * = (id, name, chatType, chatId, value, `type`) <> (VariablesRow.tupled, VariablesRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(name), Rep.Some(chatType), Rep.Some(chatId), Rep.Some(value), Rep.Some(`type`))).shaped.<>({r=>import r._; _1.map(_=> VariablesRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
