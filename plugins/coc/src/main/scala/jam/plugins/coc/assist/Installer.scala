package jam.plugins.coc.assist
...
class Installer extends JamPluginInstaller {
   ...
   * 插件名
   */
  override val pluginName: String = "COC 跑团辅助"
  /**
   * 作者名
   */
   ...
   * 关键词
   */
  override val keywords: List[String] = List("TRPG", "跑团", "COC")
  /**
   * 版本号
   *
   ...
   * 挂载点将自动中的内容会挂载到系统中
   */
  override val mountPoint: Option[MountPoint] = Some(MountPoint(
   ...
   * @return 安装器本身
   */
  override def upgrade(oldVersion: BigDecimal)(implicit exec: ExecutionContext): Future[Try[JamPluginInstaller]] = Future.successful(Success(this))
}
