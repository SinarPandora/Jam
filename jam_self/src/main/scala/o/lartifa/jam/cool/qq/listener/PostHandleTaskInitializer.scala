...
import o.lartifa.jam.common.config.SystemConfig.MessageListenerConfig.PostHandleTask
...
object PostHandleTaskInitializer {
  ...
  def tasks: List[PostHandleTask] = {
    ...
    val tasks = PostHandleTask.enabledTasks.flatMap(allTasks.get)
    if (PostHandleTask.enabledTasks.sizeIs == tasks.size) {
      logger.log(s"${AnsiColor.GREEN}已启动如下后置任务：${PostHandleTask.enabledTasks.mkString(", ")}")
    } else {
