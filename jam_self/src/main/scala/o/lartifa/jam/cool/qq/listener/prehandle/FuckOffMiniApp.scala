...
object FuckOffMiniApp extends PreHandleTask("替换小程序跳转") {
   ...
   */
  private val RICH_MESSAGE_REGEX_DOC: Regex = """\[CQ:rich,data=\{"app".+?"qqdocurl":"(?<url>.+?)".+]""".r
  private val RICH_MESSAGE_REGEX_JUMP: Regex = """\[CQ:rich,data=\{"app".+?"jumpUrl":"(?<url>.+?)".+]""".r
  private val RICH_MESSAGE_REGEX_XML: Regex = """\[CQ:rich,data=<\?xml.+?url="(?<url>.+?)".+]""".r
