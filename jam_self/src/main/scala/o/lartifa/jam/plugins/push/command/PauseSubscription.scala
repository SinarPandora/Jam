 ...
 * 2022/6/9 00:28
 */
object PauseSubscription extends Command[Unit] {
  /**
   * 执行
   *
   * @param context 执行上下文
        ...
        await(SourcePushClient.getSubscriptionOrPrompt(SourceIdentity(sourceType, sourceIdentity), context.chatInfo)) match {
          case Some(subscriber) =>
            ActorCreator.actorOf(ExtraActor(
              ctx => subscriber ! SourceSubscriberProtocol.Pause(ctx.self),
              _ => {
                case CommonProtocol.Done => reply("订阅已暂停")
              }
            ))
          case None => reply("订阅不存在！")
        }
      case (None, _) => reply(Prompts.PleaseProvideMoreInfoForCommand("暂停", "订阅源类型"))
      case (_, None) => reply(Prompts.PleaseProvideMoreInfoForCommand("暂停", "订阅源标识"))
    }
  }
}
