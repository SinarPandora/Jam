...
trait PluginsTable {
  ...
  case class PluginsRow(id: Long, name: String, keywords: String = "", author: String, `package`: String, installDate: java.sql.Timestamp, isEnabled: Boolean = false)
  /** GetResult implicit for fetching PluginsRow objects using plain SQL queries */
  implicit def GetResultPluginsRow(implicit e0: GR[Long], e1: GR[String], e2: GR[java.sql.Timestamp], e3: GR[Boolean]): GR[PluginsRow] = GR{
    prs => import prs._
    PluginsRow.tupled((<<[Long], <<[String], <<[String], <<[String], <<[String], <<[java.sql.Timestamp], <<[Boolean]))
  }
  ...
  class Plugins(_tableTag: Tag) extends profile.api.Table[PluginsRow](_tableTag, "plugins") {
    def * = (id, name, keywords, author, `package`, installDate, isEnabled) <> (PluginsRow.tupled, PluginsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(name), Rep.Some(keywords), Rep.Some(author), Rep.Some(`package`), Rep.Some(installDate), Rep.Some(isEnabled)).shaped.<>({ r=>import r._; _1.map(_=> PluginsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
