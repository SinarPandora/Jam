...
trait TrpgStatusHistoryTable {
  ...
  case class TrpgStatusHistoryRow(id: Long, statusId: Long, actorId: Long, gameId: Long, attrAdjust: String = "{}", status: String = "{}", tags: String = "{}", updateDate: java.sql.Timestamp)
  ...
  implicit def GetResultTrpgStatusHistoryRow(implicit e0: GR[Long], e1: GR[String], e2: GR[java.sql.Timestamp]): GR[TrpgStatusHistoryRow] = GR{
    prs => import prs._
      TrpgStatusHistoryRow.tupled((<<[Long], <<[Long], <<[Long], <<[Long], <<[String], <<[String], <<[String], <<[java.sql.Timestamp]))
  }
  ...
  class TrpgStatusHistory(_tableTag: Tag) extends profile.api.Table[TrpgStatusHistoryRow](_tableTag, "trpg_status_history") {
    def * = (id, statusId, actorId, gameId, attrAdjust, status, tags, updateDate) <> (TrpgStatusHistoryRow.tupled, TrpgStatusHistoryRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(statusId), Rep.Some(actorId), Rep.Some(gameId), Rep.Some(attrAdjust), Rep.Some(status), Rep.Some(tags), Rep.Some(updateDate)).shaped.<>({ r=>import r._; _1.map(_=> TrpgStatusHistoryRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
