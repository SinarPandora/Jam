...
trait TrpgGameTable {
  ...
  class TrpgGame(_tableTag: Tag) extends profile.api.Table[TrpgGameRow](_tableTag, "trpg_game") {
    ...
    def ? = (Rep.Some(id), Rep.Some(name), Rep.Some(ruleName), Rep.Some(kpList), lastChat).shaped.<>({ r=>import r._; _1.map(_=> TrpgGameRow.tupled((_1.get, _2.get, _3.get, _4.get, _5)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
