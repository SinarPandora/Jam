...
trait SourceObserverTable {
  ...
  case class SourceObserverRow(id: Long, sourceIdentity: String, sourceType: String, createTime: java.sql.Timestamp, isActive: Boolean = true)
  ...
  class SourceObserver(_tableTag: Tag) extends profile.api.Table[SourceObserverRow](_tableTag, "source_observer") {
    def * = (id, sourceIdentity, sourceType, createTime, isActive) <> (SourceObserverRow.tupled, SourceObserverRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(sourceIdentity), Rep.Some(sourceType), Rep.Some(createTime), Rep.Some(isActive)).shaped.<>({ r => import r.*; _1.map(_ => SourceObserverRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
