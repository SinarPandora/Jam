...
trait StorySaveInheritTable {
  ...
  class StorySaveInherit(_tableTag: Tag) extends profile.api.Table[StorySaveInheritRow](_tableTag, "story_save_inherit") {
    ...
    def ? = (Rep.Some(id), Rep.Some(legacyStoryId), Rep.Some(inUseStoryId)).shaped.<>({ r => import r._; _1.map(_ => StorySaveInheritRow.tupled((_1.get, _2.get, _3.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
