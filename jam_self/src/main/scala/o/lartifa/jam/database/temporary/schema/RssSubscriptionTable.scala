...
trait RssSubscriptionTable {
  ...
  class RssSubscription(_tableTag: Tag) extends profile.api.Table[RssSubscriptionRow](_tableTag, "rss_subscription") {
    ...
    def ? = ((Rep.Some(source), Rep.Some(sourceCategory), Rep.Some(subscribers), Rep.Some(channel), Rep.Some(lastKey), Rep.Some(lastUpdate))).shaped.<>({r=>import r._; _1.map(_=> RssSubscriptionRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
