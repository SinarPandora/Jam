...
trait StorySaveFileTable {
  ...
  class StorySaveFile(_tableTag: Tag) extends profile.api.Table[StorySaveFileRow](_tableTag, "story_save_file") {
    ...
    def ? = (Rep.Some(id), Rep.Some(storyId), Rep.Some(chatId), Rep.Some(saveList), Rep.Some(chatType), Rep.Some(chatScopeDefaultConfig)).shaped.<>({ r=>import r._; _1.map(_=> StorySaveFileRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
