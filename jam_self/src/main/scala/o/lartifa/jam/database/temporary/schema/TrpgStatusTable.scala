...
trait TrpgStatusTable {
  ...
  case class TrpgStatusRow(id: Long, actorId: Long, gameId: Long, attrAdjust: String = "{}", status: String = "{}", tags: String = "{}", updateDate: java.sql.Timestamp)
  ...
  class TrpgStatus(_tableTag: Tag) extends profile.api.Table[TrpgStatusRow](_tableTag, "trpg_status") {
    def * = (id, actorId, gameId, attrAdjust, status, tags, updateDate) <> (TrpgStatusRow.tupled, TrpgStatusRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(actorId), Rep.Some(gameId), Rep.Some(attrAdjust), Rep.Some(status), Rep.Some(tags), Rep.Some(updateDate)).shaped.<>({ r=>import r._; _1.map(_=> TrpgStatusRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
