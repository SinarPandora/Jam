...
trait SourceSubscriberTable {
  ...
  case class SourceSubscriberRow(id: Long, chatId: Long, chatType: String, sourceId: Long, createTime: java.sql.Timestamp, lastKey: String = "INIT", lastUpdateTime: java.sql.Timestamp, isActive: Boolean = true)

  /** GetResult implicit for fetching SourceSubscriberRow objects using plain SQL queries */
  implicit def GetResultSourceSubscriberRow(implicit e0: GR[Long], e1: GR[String], e2: GR[java.sql.Timestamp], e3: GR[Boolean]): GR[SourceSubscriberRow] = GR {
    prs =>
      import prs.*
      SourceSubscriberRow.tupled((<<[Long], <<[Long], <<[String], <<[Long], <<[java.sql.Timestamp], <<[String], <<[java.sql.Timestamp], <<[Boolean]))
  }
  ...
  class SourceSubscriber(_tableTag: Tag) extends profile.api.Table[SourceSubscriberRow](_tableTag, "source_subscriber") {
    def * = (id, chatId, chatType, sourceId, createTime, lastKey, lastUpdateTime, isActive) <> (SourceSubscriberRow.tupled, SourceSubscriberRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(chatId), Rep.Some(chatType), Rep.Some(sourceId), Rep.Some(createTime), Rep.Some(lastKey), Rep.Some(lastUpdateTime), Rep.Some(isActive)).shaped.<>({ r => import r.*; _1.map(_ => SourceSubscriberRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
