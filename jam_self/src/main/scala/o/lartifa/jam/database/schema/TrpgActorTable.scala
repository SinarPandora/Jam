...
trait TrpgActorTable {
  ...
  class TrpgActor(_tableTag: Tag) extends profile.api.Table[TrpgActorRow](_tableTag, "trpg_actor") {
    ...
    def ? = (Rep.Some(id), Rep.Some(name), Rep.Some(qid), Rep.Some(attr), Rep.Some(info), Rep.Some(defaultConfig), Rep.Some(isActive)).shaped.<>({ r => import r.*; _1.map(_ => TrpgActorRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
